...
str_cv2stage <- function(dataset, matG, k, genPopMap){
  genotypes <- dataset$genotype
  ...
  ## Create stratified folds with the caret package:
  folds <- caret::createFolds(genPopMap$subpop, k = k, list = TRUE, 
                              returnTrain = FALSE)
  
  valFolds <- lapply(1:k, function(i) genPopMap$genotype[folds[[i]]])
...
##### Testing stratified cross-validation #####

str_cv2stageMT <- function(dataset, matG, k, genPopMap){
  # Unique genotypes in the dataset
  genotypes <- unique(dataset$genotype)
